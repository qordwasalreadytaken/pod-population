...
//  const data = await loadJSONL("./data/social/2026-06.jsonl");
  ...
  const labels = currentData.map(d => tsLabel(d.timestamp));

  const online = currentData.map(d => d.totals.server_players);
  const publicPlayers = currentData.map(d => d.totals.public_players);
  const privatePlayers = currentData.map(d => d.totals.private_players_est);

  const avgPublic = currentData.map(d => d.metrics.avg_public_players_per_game);
  const avgPrivate = currentData.map(d => d.metrics.avg_private_players_per_game_est);

  const introvert = currentData.map(d => d.metrics.introvert_index);
...
//  const sevenDayOnline = avg(last7Days.map(d => d.totals.server_players));
  ...
  const last7Days = currentData.filter(d =>
  ...
  if (currentData.length >= 2) {
      const previous = currentData.at(-2);
  ...
  renderSummary(latest, currentData, {
    sevenDayPublicOcc,
    sevenDayPrivateOcc,
    sevenDayIntrovert
  });
